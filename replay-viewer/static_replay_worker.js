...
function stageNote() {
  ...
  try {
    var length = Module._magent_stage_len ? Module._magent_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._magent_stage_ptr();
...
function runtimeError() {
  var length = Module._magent_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._magent_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._magent_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._magent_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._magent_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._magent_load_replay(pointer, length);
    });
...
function applyInputNow() {
  ...
  try {
    if (Module._magent_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._magent_mismatch_tick()
...
importScripts('./wire_constants.js', './broadcast_core.js', './magent_replay.js');
